...
namespace WpfApp1.Updater2
{
    public class QubedotUpdater
    {
        ...
        //private readonly Uri UpdateXmlLocation = new Uri("https://raw.githack.com/dennisripp/Testupdater/master/Updater/project.xml");
        ...
        /// <summary>
        /// Creates a new QubedotUpdater object
        /// </summary>
        ...
        public QubedotUpdater(Assembly a, MainWindow owner, Uri XMLOnServer)
        {
            ParentForm = owner;
            ParentAssembly = a;
            ...
            if (!e.Cancelled)
            {
                ...
                if (JobsFromXML != null)
                {
                    ...
                    foreach (int i in validJobs)
                    {
                        ...
                        if (new QubedotUpdateAcceptForm(LocalApplicationInfos[i], JobsFromXML[i], count, validJobs.Count).ShowDialog(LocalApplicationInfos[0].Context) == MessageBoxResult.Yes)
                        {
                            acceptJobs++;
        ...
        /// <param name="applicationInfo">An QubedotUpdateInfo object containing application's info</param>
        private void DownloadUpdate(QubedotUpdateXml update, QubedotUpdateLocalAppInfo applicationInfo)
        {
            ...
            MessageBoxResult result = form.ShowDialog(applicationInfo.Context);
            ...
            else if (result == MessageBoxResult.Abort)
            {
                MessageBoxEx.Show(ParentForm, "The update download was cancelled.\nThis program has not been modified.", "Update Download Cancelled", MessageBoxButton.OK, MessageBoxImage.Information);
            }
            else
            {
        ...
        {
            MessageBoxEx.Show(ParentForm, "Application will be closed", "Success", 4000);
            UpdateApplications();
            Application.Close();
        }
